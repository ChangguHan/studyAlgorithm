...
int chk(vector<char> v, int score, int idx, int nodi) {
    ...
    if(v[idx]=='-') {
        if(idx==0) {
            int rs=0;
            if(trie[nodi].child[1]!=-1) rs += chk(v,score,idx+1, trie[nodi].child[1]);
            if(trie[nodi].child[2]!=-1) rs += chk(v,score,idx+1, trie[nodi].child[2]);
            if(trie[nodi].child[3]!=-1) rs += chk(v,score,idx+1, trie[nodi].child[3]);
            return rs;
